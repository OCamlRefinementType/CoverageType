...
let[@library] list_sized ?r:(_ = M (wf_slice v : [%v: slice]))
    ?r:(n = ((0 <= v : [%v: int]) [@over])) =
  M (wf_slice_list v && list_len v == n : [%v: slice list])

let[@library] list_size ?r:(_ = M (wf_expr v : [%v: expr]))
    ?r:(n = ((0 <= v : [%v: int]) [@over])) =
  M (wf_expr_list v && list_len v == n : [%v: expr list])

let literal : literal gen =
  let (l_int : literal gen) = nat_gen >|= fun (i : int) -> L_Int i in
  ...
  let (l_bv : literal gen) =
    bitvector_gen >|= fun (bv : Bitvector.t) -> L_BitVector bv
  in
  ...
  if n < 0 then return Err
    ...
    let (e_cond : expr gen) =
      if n < 4 then return Err
        ...
        split3 (n - 1) >>= fun (n1n2n3 : (int * int) * int) ->
        let (n1n2 : int * int), (n3 : int) = n1n2n3 in
        let (n1 : int), (n2 : int) = n1n2 in
        expr n1 >>= fun (e1 : expr) ->
        expr n2 >>= fun (e2 : expr) ->
        expr n3 >|= fun (e3 : expr) -> E_Cond (e1, e2, e3)
    in
    let (e_slice : expr gen) =
      if n < 2 then return Err
        ...
        let (n1 : int), (n2 : int) = n1n2 in
        expr n1 >>= fun (e1 : expr) ->
        slices_gen expr n2 >|= fun (slices : slice list) -> E_Slice (e1, slices)
    in
