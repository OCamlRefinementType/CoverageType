...
Module Goal.
  ...
  Theorem goal : forall (inv : Z), inv >= 0 -> ((forall (h : Z), (h >= 0 /\ (True -> (h + h) = inv) /\ (False -> ((h + h) + 1) = inv)) -> (forall (v : rbtree Z), (h = 0 /\ ~rb_root_color v False /\ ~rb_root_color v True /\ num_black v h /\ no_red_red v /\ (True -> ~rb_root_color v True) /\ (False -> (h = 0 -> ~rb_root_color v False))) -> (h = 0 /\ rb_leaf v /\ (True -> ~rb_root_color v True) /\ (False -> (h = 0 -> ~rb_root_color v False))))) /\ (forall (h_0 : Z), (h_0 >= 0 /\ (False -> (h_0 + h_0) = inv) /\ (True -> ((h_0 + h_0) + 1) = inv)) -> (forall (v_0 : rbtree Z), True))).
  Proof.
    intros inv Hinv. split.
    - intros h [Hh [Ht Hf]] v H. decompose [and] H.
      intuition. subst.
