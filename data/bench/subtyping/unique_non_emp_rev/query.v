...
Module Axioms : Signatures.
  ...
  Proof.
    intros [] l1 [Ht Hu].
    - contradiction.
    - simpl in Ht. simpl in Hu. subst. intuition.
  Qed.
...
Module Goal.
  ...
  Theorem goal : forall (s : Z), 0 <= s -> (forall (v : list Z), (exists (x2 : Z), exists (s_15 : Z), exists (l2 : list Z), s > 0 /\ s_15 >= 0 /\ s_15 < s /\ s_15 = (s - 1) /\ len l2 s_15 /\ uniq l2 /\ ~list_mem l2 x2 /\ ~emp v /\ len v s /\ uniq v) -> (exists (x1 : Z), exists (l1 : list Z), s > 0 /\ (s - 1) >= 0 /\ (s - 1) < s /\ len l1 (s - 1) /\ uniq l1 /\ ~list_mem l1 x1 /\ hd v x1 /\ tl v l1)).
  Proof.
    intros s Hs v [x [s1 [l H]]].
    intuition. subst.
    destruct (list_no_emp_exists_hd v) as [x1 Hh].
    destruct (list_no_emp_exists_tl v) as [t Ht].
    exists x1. exists t. (* exists x. exists l. *)
    intuition.
    - apply (list_tl_len_plus_1 v t (s - 1) H8).
      replace (s - 1 + 1) with s by lia.
      assumption.
    - apply (list_tl_unique v t). intuition.
    - apply (list_hd_unique v t x1) in H10. contradiction.
      intuition.
  Qed.
End Goal.
