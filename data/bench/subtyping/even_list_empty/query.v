...
Module Axioms : Signatures.
  ...
  Proof.
    ...
    - contradiction.
  Qed.
  Lemma list_emp_all_even : forall (l : list Z), emp l -> all_evens l.
  Proof.
    intros [| x] He.
    - reflexivity.
    - contradiction.
...
Module Goal.
  ...
  Theorem goal : forall (s : Z), 0 <= s -> (forall (v : list Z), (s = 0 /\ emp v) -> (exists (n : Z), len v n /\ n <= s /\ s = 0 /\ all_evens v)).
  Proof.
    intros s Hs v [Hse He].
    exists 0.
    intuition.
    apply (list_len_0_emp v He).
    apply (list_emp_all_even v He).
  Qed.
End Goal.
