...
Module Type Signatures.
  ...
  Axiom rbtree_rb_leaf_no_rb_root_color : forall (l : rbtree Z) (x : bool), (rb_leaf l) -> ~(rb_root_color l x).
...
Module Axioms : Signatures.
  ...
  Lemma rbtree_rb_leaf_no_rb_root_color : forall (l : rbtree Z) (x : bool), (rb_leaf l) -> ~(rb_root_color l x).
  Proof.
    intros [] x Hl.
    - intros [].
    - contradiction.
...
Module Goal.
  ...
  Theorem goal : forall (inv : Z), inv >= 0 -> ((forall (h : Z), (h >= 0 /\ (True -> (h + h) = inv) /\ (False -> ((h + h) + 1) = inv)) -> (forall (v : rbtree Z), True)) /\ (forall (h_0 : Z), (h_0 >= 0 /\ (False -> (h_0 + h_0) = inv) /\ (True -> ((h_0 + h_0) + 1) = inv)) -> (forall (v_0 : rbtree Z), (h_0 = 0 /\ rb_leaf v_0 /\ (False -> ~rb_root_color v_0 true) /\ (True -> (h_0 = 0 -> ~rb_root_color v_0 false))) -> (h_0 = 0 /\ ~rb_root_color v_0 false /\ ~rb_root_color v_0 true /\ num_black v_0 h_0 /\ no_red_red v_0 /\ (False -> ~rb_root_color v_0 true) /\ (True -> (h_0 = 0 -> ~rb_root_color v_0 false)))))).
  Proof.
    intros inv Hinv. split.
    - intros h [Hh [Ht Hf]]. reflexivity.
    - intros h [Hh [Ht Hf]] v [Hvh [Hl [Hvt Hvf]]].
      intuition.
      * apply (rbtree_rb_leaf_no_rb_root_color _ _ Hl) in H0. contradiction.
