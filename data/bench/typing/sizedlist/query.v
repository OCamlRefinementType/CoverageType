...
Module Goal.
  ...
  Theorem goal : forall (s : Z), 0 <= s -> (forall (v : list Z), (exists (n : Z), len v n /\ n <= s) -> ((~s = 0 <-> s > 0) /\ ((s = 0 /\ emp v) \/ (~s = 0 /\ (emp v \/ (exists (_x_6 : list Z), (s - 1) < s /\ 0 <= (s - 1) /\ (exists (n_2 : Z), len _x_6 n_2 /\ n_2 <= (s - 1)) /\ (exists (_x_7 : Z), hd v _x_7 /\ tl v _x_6))))))).
  Proof.
    intros s Hs v [n [Hl Hnl]].
    apply list_len_geq_0 in Hl as Hnl1.
    split.
    - intuition.
    - destruct (s =? 0) eqn:Hse.
      * left.
        assert (n = 0) by intuition. subst.
        destruct (list_len_0_emp_iff v).
        intuition.
      * right. intuition. destruct (n =? 0) eqn:Hne.
        + left. assert (n = 0) by intuition. subst.
          destruct (list_len_0_emp_iff v).
          intuition.
        + right.
          assert (n > 0) by intuition.
          assert (~emp v) by (apply (list_positive_len_is_not_emp v n); intuition).
          destruct (list_no_emp_exists_tl v) as [v1 Htl].
          destruct (list_no_emp_exists_hd v) as [x Hh].
          exists v1. intuition.
          -- exists (n - 1).
             destruct (list_tl_len_plus_1 v v1 (n - 1)); try assumption.
             replace (n - 1 + 1) with n in H4 by intuition.
             intuition.
          -- exists x. intuition.
  Qed.
End Goal.
